...
export function ResumoAcademicoCard({ resumo }: { resumo: ProfessorResumo | null }) {
  ...
  return (
    <div className="glow-card rounded-xl p-4 border border-indigo-900/40 bg-indigo-950/10 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
      <div>
        ...
        <p className="text-slate-200 font-semibold mt-1 capitalize">
      ...
      <div>
        ...
        <p className="text-slate-200 font-semibold mt-1">{resumo.orientacoes_ultimos_5_anos}</p>
      </div>
      <div>
        <span className="text-[9px] text-slate-500 uppercase font-bold flex items-center gap-1">
          <BookOpen className="w-3 h-3" /> Formações
        </span>
        <p className="text-slate-200 font-semibold mt-1">{resumo.total_formacoes} registros</p>
      </div>
    </div>
  );
}
