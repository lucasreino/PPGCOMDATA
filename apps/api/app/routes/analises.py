...
@router.get("/artigos/qualis")
async def obter_artigos_qualis(
    professor_id: Optional[str] = None,
    linha_pesquisa_id: Optional[str] = None,
    ano_inicio: Optional[int] = None,
    ano_fim: Optional[int] = None,
    ...
    """Painel de artigos: percentual por estrato Qualis, revistas e docente × estrato."""
    try:
        cache_key = (
            "analytics:artigos-qualis:"
        ...
        def _load():
            return build_artigos_qualis_insights(
                session,
        ...
        logger.error("Erro ao computar painel Qualis de artigos: %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
